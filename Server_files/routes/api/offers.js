...
router.get("/profile/owner/get", tokenCheck, async (req,res) => {
    try{
      ...
      const get_result = await db.query(
        ...
        LOWER(ba.owner) = LOWER($1)",[username]);
  ...
  router.post("/profile/insert", tokenCheck, async (req,res) => {
    try {
      ...
      const get_books_id = await db.query("SELECT books_id FROM books WHERE LOWER(title) = LOWER($1)",[book_name]);
      const get_book_active_id = await db.query("SELECT ba.book_active_id from books_active ba INNER JOIN books b ON\
      ba.books_id = $1 AND LOWER(ba.owner) = LOWER($2)",[get_books_id.rows[0].books_id, owner_name]);
      const get_result = await db.query("INSERT INTO offers(book_active_id,renter) values($1,$2) RETURNING *",
  ...
  router.get("/profile/renter/get", tokenCheck, async(req,res) => {
    try {
      const username = req.user;
      const get_result = await db.query("SELECT b.title,ba.owner from books b INNER JOIN books_active ba ON \
  ...
  router.put("/profile/delete", tokenCheck, async(req,res) => {
    try {
      const username = req.user;
      const {book_name, owner_name} = req.body;
      const get_result = await db.query("DELETE FROM offers where renter=$1 AND \
